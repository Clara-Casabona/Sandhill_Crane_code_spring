...
#load("hab_dyn2.Rdata") 
#load("MHo_p_MAt.sMAt2_MHo_p4_gof.Rdata")
#load("m1_4_gof.Rdata")
load("../data/MHo_p_MAt.sMAt2_MHo_p.Rdata")
...
# Standardisation variables SITE

SiteCovs_s <- SiteCovs %>% mutate_each_(funs(scale(.) %>% as.vector), vars=c( "MHo_p","MU_p", "mm", "temp", "LONG", "MAi_s", "MAt_s"))

SiteCovs_s$MAt_2 <- SiteCovs_s$MAt_s^2 # Creation variable quadratique
SiteCovs_s$MAi_2 <- SiteCovs_s$MAi_s^2 # Creation variable quadratique

# Standardisation variables qui changent annuelles: 
MAi <- as.matrix(YearCovs_MAi)
MAi.mean <- mean(MAi, na.rm = TRUE) 
MAi.sd <- sd(MAi, na.rm = TRUE)
MAi.s <- (MAi-MAi.mean)/MAi.sd
MAi2 <- MAi.s^2 # Creation variable quadratique

MAt <- as.matrix(YearCovs_MAt)
MAt.mean <- mean(MAt, na.rm = TRUE) 
MAt.sd <- sd(MAt, na.rm = TRUE)
MAt.s <- (MAt-MAt.mean)/MAt.sd
MAt2 <- MAt.s^2 # Creation variable quadratique


# eff - min echantillonage
...
```{r Selection AIC, eval=FALSE, include=FALSE}
...
Test GOF (Runing this take several hours)
```{r test ajustement 1, eval=FALSE, include=FALSE}
...
#gof2 <- mb.gof.test(m1, nsim = 1000, plot.hist = TRUE) 
#save(gof2, file = "m1_dyn_gof.Rdata")year.stdyear.std
#gof <- read_file("m1_dyn_gof.Rdata")
...
```{r}
## Detection data
nd <- data.frame(Met_eff=c('ALT_E','ATL_F','EBIRD_E','EBIRD_F','HHT'), JJ = 0)
E.det <- predict( m1, type='det', newdata=nd)
save(E.det, file = "E.det.Rdata")
with(E.det, {
  plot(1:5, Predicted, pch=19, xaxt='n', xlab='Sampling methodology splited by effort',
       ylab=expression(paste('Detection probability ( ', p, ' )')),
       ylim=c(0,1), col= c("#9999FF", "#9999FF", "#99FF99", "#99FF99", "#A0A0A0"),
       xlim = c(0.5,5.5), 
       cex.axis = .9,
       cex.lab = .8)
  axis(1, at=1:5, labels=c("Atlas_high", "Atlas_low","eBird_high", "eBird_low", "CWS"),cex.axis = .7 )
  arrows(1:5, lower, 1:5, upper, code=3, angle=90, length=0.05, col=  c("#9999FF", "#9999FF", "#99FF99", "#99FF99", "#A0A0A0"))
})

```

Step 2: Habitat models 

Runing habitat models (This might take 1-2 hours)

```{r modeles dynamiques habitat - H1, eval=FALSE, include=FALSE}
# Il y a une liste de 64 modèles possibles pour l'habitat qui provienent du fichier MOD_dyn_hab.csv
...
MHo_p_MAt.sMAt2_MHo_p <-colext(~MHo_p,~MAt.s+MAt2,~MHo_p,~JJ+Met_eff,grue.data) # This was the top ranked model using our original data - set
#save(MHo_p_MAt.sMAt2_MHo_p, file = "MHo_p_MAt.sMAt2_MHo_p.Rdata")
#load("MHo_p_MAt.sMAt2_MHo_p.Rdata")
...
#MHo_p_MAt.sMAt2_MHo_p_longyear <-colext(~MHo_p,~MAt.s+MAt2 ,~MHo_p + year + LONG + MHo_p*year,~JJ+Met_eff,grue.data)
#MHo_p_MAt.sMAt2_MHo_p_longyear2 <-colext(~MHo_p,~MAt.s+MAt2 ,~MHo_p + year + LONG + MHo_p*LONG,~JJ+Met_eff,grue.data)


```

AIC selection for habitat models
...
#Model detection
nd <- data.frame(Met_eff=c('ALT_E','ATL_F','EBIRD_E','EBIRD_F','HHT'), JJ = 1)
E.det <- predict( MHo_p_MAt.sMAt2_MHo_p
, type='det', newdata=nd)

with(E.det, { # Plot for detection probability: note 5 cathegoriesrs
  plot(1:5, Predicted, pch=19, xaxt='n', xlab='Base de donn?es (+ effort)',
       ylab=expression(paste('Probabilit? de d?tection ( ', p, ' )')),
       ylim=c(0,1), col= c("red", "red", "green", "green", "blue"),
       xlim = c(0.5,5.5), 
       main = "Probabilit? de d?tection selon le jeu de donn?es et l'effort")
  axis(1, at=1:5, labels=nd$Met)
  arrows(1:5, lower, 1:5, upper, code=3, angle=90, length=0.05, col=  c("red", "red", "green", "green", "blue"))
})


# Trouver mean et sd pour faire la colonne non standardise
...
# plot graphyque occupancy MH., MU.... etc
...
with(E.psi, {
plot(MHo, Predicted, ylim=c(0,1), type="l",
xlab="% Open wetlands",
ylab=expression(paste('Initial occupancy probability ( ', psi, ' )')),
cex.lab=0.8, cex.axis=0.8)
polygon(c(rev(MHo), MHo), c(rev(Predicted+1.96*SE), Predicted-1.96*SE), col = '#CCFFCC', border = NA)
  lines(MHo, Predicted, col="black")
})
...
ma$MAt2 <- ma$MAt.s^2
E.ext <- predict(MHo_p_MAt.sMAt2_MHo_p, type="col", newdata=ma, appendData=TRUE)
with(E.ext, {
plot(MAt, Predicted, ylim=c(0,1), type="l",
xlab="% Agricultural landscape",
ylab=expression(paste('Colonization probability ( ', gamma, ' )')),
cex.lab=0.8, cex.axis=0.8)
polygon(c(rev(MAt), MAt), c(rev(Predicted+1.96*SE), Predicted-1.96*SE), col = '#CCFFCC', border = NA)
  lines(MAt, Predicted, col="black")
})


with(E.ext, {
plot(MAt, Predicted, ylim=c(0,0.1), type="l",
xlab="% Agricultural landscape",
ylab=expression(paste('Colonization probability ( ', gamma, ' )')),
cex.lab=0.8, cex.axis=0.8)
  polygon(c(rev(MAt), MAt), c(rev(Predicted+1.96*SE), Predicted-1.96*SE), col = '#CCFFCC', border = NA)
...
Step 2: Expansion models

```{r modeles dynamiques LONG, eval=FALSE, include=FALSE}
...
LONG.DATA04 <- data.frame(LONG.t = seq(from=min(SiteCovs$LONG, na.rm = TRUE),
                           to = max(SiteCovs$LONG,na.rm = TRUE),
                           length=3600),
                JJ = 0,     
                 year = - 1.626948,
                       Met_eff = factor("HHT",levels =levels(grue.data@obsCovs$Met_eff)))

LONG.DATA04$LONG <- (LONG.DATA04$LONG.t-LONG.mean)/LONG.sd 



LONG.DATA12 <- data.frame(LONG.t = seq(from=min(SiteCovs$LONG, na.rm = TRUE),
                           to = max(SiteCovs$LONG,na.rm = TRUE),
                           length=3600),
                JJ = 0,     
                 year = 0.1084632,
...
LONG.DATA12$LONG <- (LONG.DATA12$LONG.t-LONG.mean)/LONG.sd 


LONG.DATA19 <- data.frame(LONG.t = seq(from=min(SiteCovs$LONG, na.rm = TRUE),
                           to = max(SiteCovs$LONG,na.rm = TRUE),
                           length=3600),
                JJ = 0,     
                 year = 1.626948,
...
LONG.DATA19$LONG <- (LONG.DATA19$LONG.t-LONG.mean)/LONG.sd 


# Creation tableaux pour graphique

E.col04 <- predict(LONGYEAR2, type="col", newdata=LONG.DATA04, appendData=TRUE)
E.col12 <- predict(LONGYEAR2, type="col", newdata=LONG.DATA12, appendData=TRUE)
E.col19 <- predict(LONGYEAR2, type="col", newdata=LONG.DATA19, appendData=TRUE)
# Creation nouvelle colonne ann?e puis unification des trois tableaux
E.col04$Year <- "2004"
E.col12$Year <- "2012"
E.col19$Year <- "2019"

E.col0412 <- full_join(E.col04,E.col12)
E.col <- full_join(E.col0412,E.col19)
...
E.col %>%
  ...
  geom_smooth_ci(Year) +
  theme_bw() +
  xlab("Longitude") + ylab(expression(paste('Probabilit? de colonisation ( ', gamma, ' )'))) +
  theme(
...
E.ext04 <- predict(LONGYEAR2, type="ext", newdata=LONG.DATA04, appendData=TRUE)
E.ext12 <- predict(LONGYEAR2, type="ext", newdata=LONG.DATA12, appendData=TRUE)
E.ext19 <- predict(LONGYEAR2, type="ext", newdata=LONG.DATA19, appendData=TRUE)
# Creation nouvelle colonne ann?e puis unification des trois tableaux
E.ext04$Year <- "2004"
E.ext12$Year <- "2012"
E.ext19$Year <- "2019"

E.ext0412 <- full_join(E.ext04,E.ext12)
E.ext <- full_join(E.ext0412,E.ext19)
...
E.ext %>%
  ...
  geom_smooth_ci(Year) +
  theme_bw()+
  ggtitle("Probabilit? d extinction selon la longitude ") +
  xlab("Longitude") + ylab(expression(paste('Probabilit? d extinction ( ', epsilon, ' )'))) +
  theme(
axis.title.x = element_text(color="black", size=10, face="bold"),
axis.title.y = element_text(color="black", size=10, face="bold")
)
...
#AUTRES FA?ONS DE PREDIRE - Utiliser ?a!


par(mfrow=c(1,1))
with(c(E.col04), {
 
  plot(LONG.t, Predicted, ylim=c(0,1), xlim=c(-79,-64),type="l", 
       main = "A",
       xlab="Longitude",
       ylab=expression(paste('colonisation probability ( ', gamma, ' ) in 2004')),
       cex.lab=1.1, cex.axis=1.1)
  polygon(c(rev(LONG.t), LONG.t), c(rev(Predicted+1.96*SE), Predicted-1.96*SE), col = '#CCFFCC', border = NA)
  # intervals
  lines(LONG.t, Predicted, col="black")
...
with(c(E.col12), {
  ...
  plot(LONG.t, Predicted, ylim=c(0,1), xlim=c(-79,-64),type="l",
       xlab="Longitude",
       ylab=expression(paste('colonisation probability ( ', gamma, ' ) in 2012')),
        +  main= "A",
       cex.lab=0.8, cex.axis=0.8)
  polygon(c(rev(LONG.t), LONG.t), c(rev(Predicted+1.96*SE), Predicted-1.96*SE), col = '#CCFFCC', border = NA)
  # intervals
...
with(c(E.col19), {
  ...
  plot(LONG.t, Predicted, ylim=c(0,1), xlim=c(-79,-64),type="l",
       xlab="Longitude",
       ylab=expression(paste('colonisation probability ( ', gamma, ' ) in 2019')),
       cex.lab=0.8, cex.axis=0.8)
  polygon(c(rev(LONG.t), LONG.t), c(rev(Predicted+1.96*SE), Predicted-1.96*SE), col = '#CCFFCC', border = NA)
  # intervals
  lines(LONG.t, Predicted, col="black")
...
with(c(E.ext04), {
  ...
  plot(LONG.t, Predicted, ylim=c(0,1), xlim=c(-79,-64),type="l",
       xlab="Longitude",
       ylab=expression(paste('extinction probability ( ', epsilon, ' ) in 2004')),
       cex.lab=0.8, cex.axis=0.8)
  polygon(c(rev(LONG.t), LONG.t), c(rev(Predicted+1.96*SE), Predicted-1.96*SE), col = '#CCFFCC', border = NA)
  # intervals
  lines(LONG.t, Predicted, col="black")
...
with(c(E.ext12), {
  ...
  plot(LONG.t, Predicted, ylim=c(0,1), xlim=c(-79,-64),type="l",
       xlab="Longitude",
       ylab=expression(paste('extinction probability ( ', epsilon, ' ) in 2012')),
       cex.lab=0.8, cex.axis=0.8)
  polygon(c(rev(LONG.t), LONG.t), c(rev(Predicted+1.96*SE), Predicted-1.96*SE), col = '#CCFFCC', border = NA)
  # intervals
  lines(LONG.t, Predicted, col="black")
...
with(c(E.ext19), {
  ...
  plot(LONG.t, Predicted, ylim=c(0,1), xlim=c(-79,-64),type="l",
       xlab="Longitude",
       ylab=expression(paste('extinction probability ( ', epsilon, ' ) in 2019')),
       cex.lab=0.8, cex.axis=0.8)
  polygon(c(rev(LONG.t), LONG.t), c(rev(Predicted+1.96*SE), Predicted-1.96*SE), col = '#CCFFCC', border = NA)
  # intervals
  lines(LONG.t, Predicted, col="black")
...
### Occupation initialle

# 2004 

E.occ04 <- predict(LONGYEAR2, type="psi", newdata=LONG.DATA04, appendData=TRUE)

with(c(E.occ04), {
  
  plot(LONG.t, Predicted, ylim=c(0,1), xlim=c(-79,-64),type="l",
       xlab="Longitude",
       ylab=expression(paste('occupancy  ( ',psi, ' ) in 2004')),
       cex.lab=0.8, cex.axis=0.8)
  polygon(c(rev(LONG.t), LONG.t), c(rev(Predicted+1.96*SE), Predicted-1.96*SE), col = '#CCFFCC', border = NA)
  # intervals
  lines(LONG.t, Predicted, col="black")
...
# 2012 and 2019 - calculated by the function
occ05 <- predict(LONGYEAR2, type="psi", newdata=LONG.DATA04, appendData=TRUE) 


# ?a ne marche pas du tout .. E.occ12 <- predict(LONGYEAR2, type="psi", newdata=LONG.DATA12, appendData=TRUE)
